...
def get_average_scores(scores_per_fold):
...
def performance(model, list_of_X_y, list_metrics, verbose=False):
    ...
    for i in range(n_folds):
        ...
        if verbose:
            print(f"Scores for fold {i}:")
            traintest.print_scores(scores)
            print()
    if verbose:
        average_scores = get_average_scores(scores_per_fold)
